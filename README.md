...
| `grpcChannelOptions`    | Object  | Optional gRPC channel options passed directly to the gRPC client constructor. Use to configure transport-level settings such as keepalive (e.g. `grpc.keepalive_time_ms`, `grpc.keepalive_timeout_ms`, `grpc.keepalive_permit_without_calls`). |
